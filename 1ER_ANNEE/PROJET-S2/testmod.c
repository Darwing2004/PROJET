...
BigInt Addition(const BigInt *A, const BigInt *B)
{
    ...
    while (resultAdd.size > 1 && resultAdd.digits[0] == 0)
    {
...
BigInt Multiplication(const BigInt *A, const BigInt *B)
{
    ...
    int sizemax = A->size + B->size + 1;
    resultMult.size = sizemax;
    temp.size = sizemax;
    ...
    while (B_i >= 0)
    {
        ...
        resultMult = Addition(&resultMult, &temp);
        ...
        B_i--;
        A_i = A->size - 1;
        result_i = temp.size - 1 - j;
        retenue = 0;
    }

    if (resultMult.digits[0] == 0)
    {
...
int main()
{
    const char *numberStrA = "9962815268583841485169";
    const char *numberStrB = "6168168418164";
